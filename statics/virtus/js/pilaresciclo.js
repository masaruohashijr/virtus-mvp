...
class Item {
	constructor(order, id, elementoId, nome, descricao, avaliacao, autorId, autorNome, dataCriacao, status, cStatus) {
		this.order = order;
		this.id = id;
...
function criarItem(){
	...
	item = new Item(0, itens.length, 0, nome, descricao, avaliacao, '', '', '', '', '');
	itens.push(item);
...
function updateItem(){
	...
	var nome = document.getElementById('NomeItemForUpdate').value;
	...
	item = new Item(order, id, elementoId, nome, descricao, avaliacao, '', '', '', '','');
	itens[order] = item;
