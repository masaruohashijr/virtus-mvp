...
func ListProdutosComponentesHandler(w http.ResponseWriter, r *http.Request) {
	log.Println("List Produtos Componentes Handler")
	if sec.IsAuthenticated(w, r) {
		currentUser := GetUserInCookie(w, r)
		...
		page.Title = "Designar Papéis"
		page.LoggedUser = BuildLoggedUser(GetUserInCookie(w, r))
		var tmpl = template.Must(template.ParseGlob("tiles/produtoscomponentes/*"))
		tmpl.ParseGlob("tiles/*")
		tmpl.ExecuteTemplate(w, "Main-Produtos-Componentes", page)
	} else {
		http.Redirect(w, r, "/logout", 301)
	}
}
